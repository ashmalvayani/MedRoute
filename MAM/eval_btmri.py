#!/usr/bin/env python3
"""Evaluate MAM on the Brain Tumor MRI dataset (image modality, 4-choice MCQ).

Brain Tumor MRI is a 4-class classification task: given a brain MRI image,
identify whether it shows {glioma, meningioma, no tumor, pituitary}.
...
Typical two-model usage:
    ...
    MAM_VLM_EXTRA_BODY='{"chat_template_kwargs":{"enable_thinking":false}}' \
        .venv/bin/python eval_btmri.py --run-name BTMRI/Qwen35-9B_Qwen3-8B --quiet
"""
...
# Reverse map: tumor_type string -> letter
TUMOR_TO_LETTER = {v: k for k, v in TUMOR_OPTIONS.items()}

# --------------------------------------------------------------------------
# Answer-letter extraction (A-D)
# --------------------------------------------------------------------------
...
def format_question(example):
    lines = [
        "Look at the provided brain MRI image and identify the condition shown.",
        "",
        "Question: What does this brain MRI show?",
        "",
        "Options:",
    ]
    for k in sorted(TUMOR_OPTIONS.keys()):
        lines.append(f"{k}. {TUMOR_OPTIONS[k].capitalize()}")
    lines.append("")
    lines.append(
        "Select the single best answer. Conclude your response with a line in "
        "exactly this format: 'Answer: <letter>' where <letter> is one of A, B, C, D."
    )
    return "\n".join(lines)
...
def extract_letter(text, options=None):
    ...
    if m:
        return m.group(1).upper()
    # Try matching tumor type names in text
    low = text.lower()
    best, best_len = None, 0
    for k, v in TUMOR_OPTIONS.items():
...
def main():
    ...
    p.add_argument("--data", default="data/btmri/test.jsonl")
    p.add_argument("--data-dir", default="data/btmri",
                    help="Base directory for image paths (default: data/btmri).")
    ...
    if args.run_name is None:
        ...
        args.run_name = f"{_safe_name(assistant.model_name)}__btmri__{stamp}"
    ...
    print(f"[eval] dataset       = BTMRI (4-class MCQ)")
    ...
    def process_one(i, ex):
        ...
        gold_type = ex["tumor_type"]
        gold_letter = TUMOR_TO_LETTER.get(gold_type)
    ...
    def emit_result(r):
        ...
        pred_type = TUMOR_OPTIONS.get(pred, "unknown") if pred else None
