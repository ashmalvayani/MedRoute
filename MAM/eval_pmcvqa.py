#!/usr/bin/env python3
"""Evaluate MAM on the PMC-VQA test_clean set (image modality, 4-choice MCQ).

PMC-VQA is a 4-option (A/B/C/D) medical visual QA task. We report accuracy
against `answer_label`.
...
Typical two-model usage:
    ...
    MAM_VLM_EXTRA_BODY='{"chat_template_kwargs":{"enable_thinking":false}}' \
        .venv/bin/python eval_pmcvqa.py --run-name Qwen3-PMCVQA --concurrency 32 --quiet
"""
...
def format_question(example):
    ...
    lines = [
        "Look at the provided medical image and answer the following question.",
        "",
        f"Question: {q}",
        "",
        "Options:",
...
def main():
    ...
    p.add_argument("--data", default="data/pmcvqa/test.jsonl")
    p.add_argument("--data-dir", default="data/pmcvqa",
                    help="Base directory for image paths (default: data/pmcvqa).")
    ...
    if args.run_name is None:
        ...
        args.run_name = f"{_safe_name(assistant.model_name)}__pmcvqa__{stamp}"
    ...
    print(f"[eval] dataset       = PMC-VQA (test_clean)")
    ...
    def emit_result(r):
        ...
        trace_record = {
            "index": idx, "gold": r["gold"], "pred": pred, "correct": ok,
