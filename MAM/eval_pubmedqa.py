#!/usr/bin/env python3
"""Evaluate MAM on the PubMedQA labeled (PQA-L) test set (text modality).

PubMedQA is a yes/no/maybe QA task grounded in PubMed abstracts. The prompt
gives the model the abstract context + question and asks for one of
{yes, no, maybe}. We report accuracy against `final_decision`.

Output layout mirrors `eval_medqa.py`:
    ...
    runs/<run-name>/
        ...
        history/              # per-run isolated memory dir
        traces/               # per-example full pipeline trace

Typical usage against a remote vLLM server:

    MAM_LLM_API_URL=http://<server-ip>:9001/v1 \
    MAM_LLM_MAX_TOKENS=2048 \
    MAM_LLM_EXTRA_BODY='{"chat_template_kwargs":{"enable_thinking":false}}' \
        .venv/bin/python eval_pubmedqa.py --run-name Qwen3-8B --concurrency 128 --quiet
"""
...
# Prefer an explicit "Answer: yes|no|maybe" line.
ANSWER_HINT = re.compile(
    r"(?:final\s+answer|answer(?:\s+is)?|conclusion|decision)"
    r"\s*[:\-]?\s*\*?\*?\(?\s*(yes|no|maybe)\b",
    re.IGNORECASE,
)
# Last-resort fallback: search for the three labels anywhere.
LABEL_ANY = re.compile(r"\b(yes|no|maybe)\b", re.IGNORECASE)
...
def format_question(example):
    """Build a single user-message prompt from a PubMedQA record."""
    q = example["question"]
    contexts = example.get("contexts") or []
    labels = example.get("context_labels") or []
    lines = ["Context:"]
    if labels and len(labels) == len(contexts):
        for lab, ctx in zip(labels, contexts):
            lines.append(f"[{lab}] {ctx}")
    else:
        for ctx in contexts:
            lines.append(ctx)
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "Based on the context above, answer the question with exactly one of: "
        "yes, no, maybe. Conclude your response with a line in exactly this "
        "format: 'Answer: <label>' where <label> is yes, no, or maybe."
    )
    return "\n".join(lines)


def extract_answer(text):
    """Recover a yes/no/maybe label from a free-form diagnosis string."""
    ...
    # Prefer the LAST mention over the first (models often state the answer at
    # the end after reasoning out loud).
    matches = LABEL_ANY.findall(text)
    if matches:
...
def main():
    ...
    p.add_argument("--data", default="data/pubmedqa/test.jsonl")
    p.add_argument(
        "--run-name",
        default=None,
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Run N PubMedQA questions in parallel (threads). 1 = sequential. "
    ...
    if args.run_name is None:
        ...
        args.run_name = f"{_safe_name(assistant.model_name)}__pubmedqa__{stamp}"
    ...
    print(f"[eval] dataset       = PubMedQA (PQA-L)")
