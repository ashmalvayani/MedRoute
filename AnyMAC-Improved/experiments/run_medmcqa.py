"""
AnyMAC-Improved: Entry point for MedMCQA router training and evaluation (4-choice MCQ).
...
Example:
  python experiments/run_medmcqa.py \
...
from datasets_my.medmcqa_dataset import MedMCQADataset
from GDesigner.prompt.medmcqa_prompt_set import ROLES, SPECIALISTS
from GDesigner.utils.const import GDesigner_ROOT
from GDesigner.utils.globals import Time

try:
    from GDesigner.domain.medmcqa_domain import MedMCQADomain
except Exception:
    MedMCQADomain = None  # type: ignore

from experiments.train_medmcqa import train
from experiments.evaluate_medmcqa import evaluate
...
def _apply_improvements(args):
    ...
    if getattr(args, 'partial_credit', False):
        import experiments.train_medmcqa as tm
...
def main():
    p = argparse.ArgumentParser(description="AnyMAC-Improved MedMCQA Router")

    p.add_argument("--result_dir", type=str, default="result/medmcqa_improved")
    ...
    p.add_argument('--domain', type=str, default="medmcqa")
    ...
    dataset_train = MedMCQADataset(getattr(args, 'train_split', 'dev'))
    dataset_val = MedMCQADataset('test')
