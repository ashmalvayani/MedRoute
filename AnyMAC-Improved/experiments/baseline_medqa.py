"""
Baseline evaluation on MedQA — no routing, direct prompt → response.
Two-phase: (1) base model generation, (2) judge scoring.

Usage:
  bash scripts/run_baseline_medqa.sh
  python experiments/baseline_medqa.py --model Qwen/Qwen3.5-9B --judge_model Qwen/Qwen3.6-27B --limit 100
  python experiments/baseline_medqa.py --answers_file result/medqa_baseline/answers_*.json
"""
...
RESULT_DIR = Path("result/medqa_baseline")

SYSTEM_PROMPT = (
    "You are a medical expert. Read the clinical question carefully and select the single best "
    "answer from the options provided. State the letter of your chosen option (A, B, C, D, or E) "
...
JUDGE_SYSTEM_PROMPT = (
    ...
    "in the question. Reply with ONLY the single letter (A–E)."
)
...
async def judge_one(
    ...
    async with semaphore:
        prompt = (
            f"Question:\n{item['Question']}\n\n"
            f"Model response:\n{item['Response']}\n\n"
            "What is the model's final answer? Reply with ONLY one letter A, B, C, D, or E."
        )
        messages = [
        ...
        try:
            ...
            letter = next((c for c in extracted if c in "ABCDE"), "")
            gt = item["Answer"].strip().upper()[:1]
            judge_correct = letter == gt if gt in "ABCDE" else False
        except Exception as e:
            verdict = f"[ERROR: {e}]"
            judge_correct = False
...
async def main_async(args):
    ...
    print(f"Dataset: MedQA ({split}) — {len(records)} questions")
...
def main():
    p = argparse.ArgumentParser(description="MedQA baseline evaluation (no routing)")
    ...
    p.add_argument(
        ...
        help="MedQA split under datasets_my/MedQA/data/<split>/",
    )
