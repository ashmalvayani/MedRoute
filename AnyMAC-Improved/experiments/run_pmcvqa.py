"""
AnyMAC-Improved: Entry point for PMC-VQA router training and evaluation.

Vision dataset pipeline:
1. Captions generated by Qwen3.5-9B (vision model) and cached per seed/sample
2. Router (GNN, text-only): sees caption + question + options as text embedding
3. Pool/prompt gen (Qwen3-32B, text-only): sees caption + question
4. Agents (Qwen3.5-9B, VLM): see actual image + question + options
...
Example:
  python experiments/run_pmcvqa.py \
    ...
    --epochs 1 --train_samples 300 --max_routing 3 --seed 99
"""
...
try:
    from GDesigner.domain.pmcvqa_domain import PMCVQADomain
except Exception:
    PMCVQADomain = None  # type: ignore
...
def build_graph(args):
    ...
    # Determine image embedding dim if using image embeddings
...
def main():
    p = argparse.ArgumentParser(description="AnyMAC-Improved PMC-VQA Router")

    p.add_argument("--result_dir", type=str, default="result/pmcvqa_improved")
    ...
    p.add_argument('--domain', type=str, default="pmcvqa")

    # Agent model: Qwen3.5-9B VLM (sees actual images)
    ...
    # Judge model: Qwen3-32B text-only (pool/prompt generation)
    ...
    # =====================================================
    # AnyMAC-Improved: Improvement flags
    # =====================================================
    ...
    p.add_argument("--eval_limit", type=int, default=2100,
                   help="Max eval questions. Default 2100 (PMC-VQA has 2000 test).")
    p.add_argument("--eval_temperature", type=float, default=0.5)
    p.add_argument("--seed", type=int, default=99)

    # =====================================================
    # Vision-specific: Caption generation
    # =====================================================
    ...
    p.add_argument("--train_samples", type=int, default=300,
                   help="Number of training samples from train_sample_2000.")
    ...
    # =====================================================
    # Vision-specific: Image embeddings for routing
    # =====================================================
    ...
    # ------------------------------------------------------------------
    # Load datasets — caption generation happens here (once, cached)
    # ------------------------------------------------------------------
    eval_only = bool(args.model_path or args.zero_shot)
    print(f"[PMC-VQA] Loading datasets (caption_model={args.caption_model}, seed={seed})...")
    use_img_emb = getattr(args, 'use_image_embeddings', False)
    vision_encoder = getattr(args, 'vision_encoder', None)
    fusion_mode = getattr(args, 'fusion_mode', 'concat')
    # caption_image needs both captions and image embeddings
    skip_captions_for_img = use_img_emb and fusion_mode != "caption_image"
    dataset_train = PMCVQADataset(
        split="train",
        ...
        skip_caption_gen=args.skip_caption_gen or eval_only or skip_captions_for_img,
        use_image_embeddings=use_img_emb,
        vision_encoder=vision_encoder,
    )
    # For eval, sample_n limits how many test questions to caption+evaluate
    # (avoids generating captions for all 2000 test images when eval_limit is small)
    eval_sample_n = limit_questions if limit_questions < 2000 else None
    dataset_val = PMCVQADataset(
        split="test",
        ...
        test_json_path=args.test_json_path,
        caption_model=args.caption_model,
        skip_caption_gen=args.skip_caption_gen or skip_captions_for_img,
        use_image_embeddings=use_img_emb,
        vision_encoder=vision_encoder,
    )
