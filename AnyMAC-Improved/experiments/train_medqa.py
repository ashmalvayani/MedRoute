"""
Training loop for MedQA (text-only MCQ) using AnyMAC's next-agent prediction RL.
...
# Global toggle — set from run_medqa.py
...
def train(
    ...
    result_dir = Path(result_dir or "./result/medqa")
    ...
    log_file_path = result_dir / f"log_train_medqa_{model_slug}_{current_time}.txt"
    ...
    with open(log_file_path, "a", encoding="utf-8") as log_file:
        ...
        for epoch in range(1, getattr(args, "epochs", 1) + 1):
            ...
            gradients_inputs_path = f"{result_dir}/all_gradients_inputs_medqa_{model_slug}_{current_time}.json"
            ...
            try:
                ...
                # graph.save_model(save_path / f"{current_time}_{getattr(args, 'llm_name', 'llm')}_medqa_model_epoch{epoch}.pth")
                graph.save_model(f"{save_path}/{current_time}_{model_slug}_medqa_model_epoch{epoch}.pth")
    ...
    # Save model
    try:
        graph.save_model(f"{result_dir}/{current_time}_{model_slug}_medqa_model.pth")
    except Exception as e:
        print(f"Error saving model: {e}")
