"""
Training loop for MedMCQA (text-only 4-choice MCQ) using AnyMAC's next-agent prediction RL.

This is based on experiments/train_mmlu.py but:
- Uses medical specialist roles (fixed pool).
- Assumes MCQ evaluation (A/B/C/D).
- Typically uses a small number of routing steps (e.g., 3) to mimic a mini-MAM meeting.
...
- graph.add_to_optimizer(...), graph.save_model(...), graph.set_eval() (trace collection and PG both use eval)

If your AnyMAC version differs, keep the training logic but adjust the calls accordingly.
"""
...
# Uses OpenAI-compatible API (/v1/chat/completions) — works with vLLM, OpenAI, etc.
# ---------------------------------------------------------------------------
...
async def judge_answer(
    ...
    payload = {
        ...
        "max_tokens": 4,
        "seed": 42,
        "chat_template_kwargs": {"enable_thinking": False},  # disable Qwen3 thinking (vLLM)
    }
    ...
    try:
        async with session.post(url, json=payload, headers=headers) as resp:
            ...
            # Compare first letter of extraction to true answer
            return len(extracted) > 0 and extracted[0] == true_answer.strip().upper()
    except Exception:
        return False

from GDesigner.prompt.medmcqa_prompt_set import ROLES, SPECIALISTS
...
# Global toggle — set from run_medmcqa.py
PARTIAL_CREDIT_ENABLED = False
REWARD_ALPHA = 0.4  # weight for binary correctness
...
def _write_trace_json(results, epoch, result_dir, model_slug, current_time, args):
    ...
    for i_q, (input_dict, true_answer, q_traces, q_rewards, q_rewards_raw) in enumerate(results):
        ...
        for r_idx, (trace_data, reward, raw) in enumerate(zip(q_traces, q_rewards, q_rewards_raw)):
            ...
            # Include hint extraction log if present
...
def train(
    ...
    result_dir = Path(result_dir or "./result/medmcqa")
    ...
    log_file_path = result_dir / f"log_train_medmcqa_{model_slug}_{current_time}.txt"

    required_correct = getattr(args, "required_correct_answers", 1)
    max_routing = getattr(args, "max_routing", 3)  # mimic 3-specialist meeting by default
    decay_factor = getattr(args, "decay_factor", 0.98)
    judge_model = getattr(args, "judge_model", None)  # e.g. "qwen3:8b-fp16" — if set, uses LLM judge for reward

    # Log training config upfront
    _train_cfg = (
    ...
    async def _sample_one_trace_async(input_dict, max_routing, temperature, agent_group_type, max_context):
        # one rollout, with timeout (same spirit as your sync wrapper)
        return await asyncio.wait_for(
            graph.arun_next_agent_prediction(
                input=input_dict,
                max_routing=max_routing,
                temperature=temperature,
                available_roles=ROLES,  # keep your current behavior
    ...
    async def _collect_traces_for_record(record, i_record, num_traces, required_correct, sem, judge_session=None):
        ...
        # keeping your prints (note: will interleave with parallel execution)
        print(input_dict)
        ...
        # launch up to num_traces tasks, but gate them with a global semaphore
        async def _run_trace():
            async with sem:
                # torch.no_grad is fine here; routing transformer forward is the only torch part
                with torch.no_grad():
        ...
        try:
            for fut in asyncio.as_completed(tasks):
                ...
                reasoning_score = 3  # neutral default
                ...
                # keep your first-trace debug logging logic
                ...
                # Early stopping: cancel remaining traces once enough correct found
                if getattr(args, 'early_stop_rollouts', False) and found_correct >= required_correct:
                    for t in tasks:
                        t.cancel()
                    break
        finally:
            # absorb cancellations/exceptions so asyncio doesn't complain
    ...
    with open(log_file_path, "a", encoding="utf-8") as log_file:
        ...
        for epoch in range(1, getattr(args, "epochs", 1) + 1):
            ...
            # Resume from saved gradient inputs (skip rollout collection for this epoch)
            ...
            if _resume_path and epoch == 1:
                ...
                # Convert JSON lists back to tensors (JSON round-trip loses torch types)
            ...
            #             if found_correct >= required_correct:
            #                 break
            ...
            #             if trace_result is None:
            #                 log_file.write("Warning: trace_result is None (timeout or failure)\n")
            #                 log_file.flush()
            #                 continue

            #             # Predicted answer (first answer string)
            #             predict_answer_list = trace_result.get("answers", [""])
            #             predict_answer_str = predict_answer_list[0] if predict_answer_list else ""
            ...
            #             # Adding logic here for is_solved
            #             is_solved = dataset.record_to_target_check(true_answer, predict_answer_str, input_dict['task'])
            #             print(f"checking is_solved: {is_solved}")

            #             # Reward: correctness * (decay by routing length)
            #             reward = (decay_factor ** routing_length) * float(is_solved)
            ...
            #             print("Printing rewards here")
            #             print(question_rewards, "\n\n")
            #             print(question_rewards_raw, "\n\n")
            ...
            #                 log_file.write(f"Routing length: {routing_length}\n")
            #                 # log the agent selections
            #                 log_file.write(f"Agent selections: {trace_result['routing_results']['agent_selections']}\n")
            #                 # log the hint selections
            #                 log_file.write(f"Hint selections: {trace_result['routing_results']['hint_selections']}\n")
            #                 log_file.flush()

            #         if training_samples < 0:
            #             print("checking training samples", training_samples)
            #             break
            #         if not question_traces:
            #             continue

            #         # Advantage normalize per question
            ...
            async def _collect_all():
                # Create a shared aiohttp session for judge calls (if using judge reward)
                judge_session = None
            ...
            if not _skip_rollouts:
                ...
                # --- Write structured trace JSON for this epoch ---
                ...
                for (input_dict, _true_ans, question_traces, question_rewards, question_rewards_raw) in results:
                    ...
                    # Advantage normalize per question (unchanged)
                    if len(question_traces) == 1:
                        advantages = np.array([0.0])
                    else:
                        baseline = float(np.mean(question_rewards))
                    ...
                    if store_traces:
                        for i_trace, trace_data in enumerate(question_traces):
                            ...
                            gi = {
                                ...
                                "trace_length": len(agent_selections),
                                "agent_outputs_embeddings": agent_outputs_embeddings,
                                "batch_record_idx": -1,  # optional: you can keep original indexing if you want
                                "trace_idx_in_record": i_trace,
                            }
                    ...
                    # keep your original accounting behavior
                    total_epoch_reward += question_rewards_raw[0]
                    trace_count += 1
            ...
            # dumping all_Gradient_inputs here
            gradients_inputs_path = f"{result_dir}/all_gradients_inputs_medmcqa_{model_slug}_{current_time}.json"
            gradients_inputs_json = {
                "epoch": epoch,
                "all_gradient_inputs": all_gradient_inputs
            }
            # if os.path.exists(gradients_inputs_path):
            #     with open(gradients_inputs_path, "r") as f:
            #         data = json.load(f)
            # else:
            #     data = []
            
            # data.append(gradients_inputs_json)
            ...
            try:
                ...
                # graph.save_model(save_path / f"{current_time}_{getattr(args, 'llm_name', 'llm')}_medmcqa_model_epoch{epoch}.pth")
                graph.save_model(f"{save_path}/{current_time}_{model_slug}_medmcqa_model_epoch{epoch}.pth")
    ...
    # Save model
    try:
        graph.save_model(f"{result_dir}/{current_time}_{model_slug}_medmcqa_model.pth")
    except Exception as e:
        print(f"Error saving model: {e}")
