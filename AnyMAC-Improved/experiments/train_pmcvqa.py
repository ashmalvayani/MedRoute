"""
Training loop for PMC-VQA (text-only MCQ) using AnyMAC's next-agent prediction RL.

Same as train_medqa.py but uses PMC-VQA specialist roles and dataset.
PMC-VQA has 4 options (A/B/C/D) for medical visual QA.
...
from experiments.evaluate_pmcvqa import judge_pathvqa_openqa
...
from GDesigner.prompt.pmcvqa_prompt_set import ROLES, SPECIALISTS
...
# Global toggle — set from run_pmcvqa.py
...
def _resolve_gradient_resume_path(raw: str, result_dir: str | Path) -> Path:
    p = Path(raw).expanduser()
    if p.is_file():
        return p
    rd = Path(result_dir)
    for c in (rd / raw, rd / Path(raw).name):
        if c.is_file():
            return c
    tried = [str(p.resolve()), str((rd / raw).resolve()), str((rd / Path(raw).name).resolve())]
    raise FileNotFoundError(
        "resume_gradient_path not found. Tried:\n  " + "\n  ".join(tried)
    )
...
def train(
    ...
    _domain = getattr(args, "domain", "pmcvqa") if args is not None else "pmcvqa"
    result_dir = Path(result_dir or f"./result/{_domain}")
    ...
    log_file_path = result_dir / f"log_train_{_domain}_{model_slug}_{current_time}.txt"
    ...
    async def _collect_traces_for_record(record, i_record, num_traces, required_correct, sem, judge_session=None):
        if hasattr(dataset, '_use_image_embeddings') and dataset._use_image_embeddings:
            input_dict = dataset.record_to_input_with_embeddings(record, fusion_mode=getattr(graph, 'fusion_mode', 'concat'))
        else:
        ...
        try:
            for fut in asyncio.as_completed(tasks):
                ...
                if judge_model and judge_session:
                    if (getattr(args, "domain", None) or "").lower() == "pathvqa":
                        q_judge = input_dict.get("task_plain", input_dict["task"])
                        is_solved = await judge_pathvqa_openqa(
                            judge_session,
                            judge_model,
                            q_judge,
                            true_answer,
    ...
    with open(log_file_path, "a", encoding="utf-8") as log_file:
        ...
        for epoch in range(1, getattr(args, "epochs", 1) + 1):
            ...
            if _resume_path and epoch == 1:
                rp = _resolve_gradient_resume_path(_resume_path, result_dir)
                print(f"\n  Resuming from saved gradient inputs: {rp}")
                log_file.write(f"Resuming from saved gradient inputs: {rp}\n")
                ...
                for (input_dict, _true_ans, question_traces, question_rewards, question_rewards_raw) in results:
                    ...
                    if store_traces:
                        for i_trace, trace_data in enumerate(question_traces):
                            ...
                            # Store image embedding and caption for gradient replay
                            if "image_embedding" in input_dict:
                                gi["image_embedding"] = input_dict["image_embedding"]
                            if "task_caption" in input_dict:
                                gi["task_caption"] = input_dict["task_caption"]
                            # Store dynamic pool info for gradient replay
                            if "dynamic_pool" in trace_data:
                                gi["dynamic_pool"] = trace_data["dynamic_pool"]
                            elif getattr(args, 'dynamic_pool', False):
                                # Skip traces where pool gen failed — no valid role mapping
                                continue
            ...
            gradients_inputs_path = str(
                Path(result_dir) / f"all_gradients_inputs_{_domain}_{model_slug}_{current_time}.json"
            )
            ...
            try:
                ...
                graph.save_model(f"{save_path}/{current_time}_{model_slug}_{_domain}_model_epoch{epoch}.pth")
    ...
    # Save final model
    try:
        graph.save_model(f"{result_dir}/{current_time}_{model_slug}_{_domain}_model.pth")
    except Exception as e:
        print(f"Error saving model: {e}")
