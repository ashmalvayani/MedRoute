"""
AnyMAC-Improved: Entry point for BTMRI (brain tumor MRI) router training and evaluation.

Vision dataset pipeline matches PMC-VQA / DeepLesion: JSON with figure_path, 4-way MCQ.
See datasets_my/btmri/btmri.json and BTMRI_JSON / --train_json_path / --test_json_path.
"""
...
from datasets_my.btmri_dataset import BtmriDataset
from GDesigner.prompt.btmri_prompt_set import SPECIALISTS
from GDesigner.utils.const import GDesigner_ROOT
from GDesigner.utils.globals import Time

try:
    from GDesigner.domain.btmri_domain import BtmriDomain
except Exception:
    BtmriDomain = None  # type: ignore
...
def main():
    p = argparse.ArgumentParser(description="AnyMAC-Improved BTMRI Router")

    p.add_argument("--result_dir", type=str, default="result/btmri_img_embed")
    ...
    p.add_argument('--domain', type=str, default="btmri")
    ...
    p.add_argument("--min_routing_depth", type=int, default=0,
                   help="Minimum specialist consultations before DM can be selected (0=no minimum)")
    ...
    p.add_argument("--caption_model", type=str, default="Qwen/Qwen3.5-9B",
                   help="Vision model for generating image captions (cached per seed/sample).")
    p.add_argument("--train_samples", type=int, default=300,
                   help="Training subset size from JSON split.")
    p.add_argument("--skip_caption_gen", action="store_true", default=False,
                   help="Skip caption generation (use only if captions are pre-cached).")

    p.add_argument("--use_image_embeddings", action="store_true", default=False,
                   help="Use image embeddings (SigLIP) for GNN routing instead of text captions.")
    p.add_argument("--vision_encoder", type=str, default="google/siglip-so400m-patch14-384",
                   help="Vision encoder model for image embeddings.")
    p.add_argument("--fusion_mode", type=str, default="concat",
                   choices=["concat", "caption_image", "cross_attention"],
                   help="How to fuse text and image embeddings: concat (default), caption_image (text+caption+image), cross_attention.")
    ...
    print(f"[BTMRI] Loading datasets (seed={seed})...")
    use_img_emb = getattr(args, 'use_image_embeddings', False)
    vision_encoder = getattr(args, 'vision_encoder', None)

    dataset_train = BtmriDataset(
        split="train",
    ...
    eval_sample_n = limit_questions
    dataset_val = BtmriDataset(
        split="test",
