"""
Baseline evaluation on MedMCQA — no routing, direct prompt → response.
Two-phase: (1) base model generation, (2) judge scoring.

Usage:
  bash scripts/run_baseline_medmcqa.sh
  python experiments/baseline_medmcqa.py --model Qwen/Qwen3.5-9B --judge_model Qwen/Qwen3.6-27B --limit 100
  python experiments/baseline_medmcqa.py --answers_file result/medmcqa_baseline/answers_*.json
"""
...
RESULT_DIR = Path("result/medmcqa_baseline")

SYSTEM_PROMPT = (
    "You are a medical expert. Read the clinical question carefully and select the single best "
    "answer from the options provided. State the letter of your chosen option (A, B, C, or D) "
...
JUDGE_SYSTEM_PROMPT = (
    ...
    "in the question. Reply with ONLY the single letter (A–D)."
)
...
async def phase1_generate(args, records, dataset) -> list[dict]:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        tasks = [
            generate_answer(
                session, args.model, rec, dataset, semaphore, i, args.max_tokens, args.no_system
            )
...
async def judge_one(
    ...
    async with semaphore:
        prompt = (
            f"Question:\n{item['Question']}\n\n"
            f"Model response:\n{item['Response']}\n\n"
            "What is the model's final answer? Reply with ONLY one letter A, B, C, or D."
        )
        messages = [
        ...
        try:
            ...
            letter = next((c for c in extracted if c in "ABCD"), "")
            gt = item["Answer"].strip().upper()[:1]
            judge_correct = letter == gt if gt in "ABCD" else False
        except Exception as e:
            verdict = f"[ERROR: {e}]"
            judge_correct = False
...
async def main_async(args):
    ...
    print(f"Dataset: MedMCQA ({split}) — {len(records)} questions")
...
def main():
    p = argparse.ArgumentParser(description="MedMCQA baseline evaluation (no routing)")
    ...
    p.add_argument(
        ...
        help="MedMCQA split under datasets_my/MedMCQA/data/<split>/",
    )
