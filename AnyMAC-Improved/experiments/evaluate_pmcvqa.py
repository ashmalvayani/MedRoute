...
from GDesigner.prompt.pmcvqa_prompt_set import SPECIALISTS
...
async def judge_pathvqa_openqa(
    ...
    """PathVQA train/eval reward judge — same contract as baseline_pathvqa phase 2."""
    prompt = (
        f"Question:\n{question_text}\n\n"
        f"Reference answer:\n{reference_answer}\n\n"
        f"Model response:\n{model_response}\n\n"
    ...
    payload = {
        "model": judge_model,
        "messages": [
            {"role": "system", "content": JUDGE_SYSTEM_PROMPT_OPENQA},
    ...
    try:
        async with session.post(url, json=payload, headers=headers) as resp:
            ...
            return openqa_judge_verdict_is_yes(content)
    except Exception:
        return False
...
def openqa_judge_verdict_is_yes(raw_content) -> bool:
    """Match baseline intent: YES counts as correct. Handles CoT / tags Qwen often emits before YES."""
    text = _strip_thinking_wrappers(_message_content_to_text(raw_content))
...
# -----------------------------
# Async parallel evaluate for PMC-VQA
# -----------------------------
def evaluate(
    graph: Graph,
    dataset,
    ...
    with open(log_file_path, "a", encoding="utf-8") as log_file:
        ...
        results_lock = asyncio.Lock() if False else None  # placeholder, we use sequential result collection

        judge_model = getattr(args, 'judge_model', None)

        async def _eval_one_sample(i_record, record, sem, judge_session, pbar):
                ...
                try:
                    ...
                    if judge_model and predict_answer_str and judge_session:
                        if (getattr(args, "domain", None) or "").lower() == "pathvqa":
                            q_judge = input_dict.get("task_plain", task)
                            is_solved_judge = await judge_pathvqa_openqa(
                                judge_session,
                                judge_model,
                                q_judge,
                                true_answer,
        ...
        async def _run_all_eval():
            ...
            try:
                tasks = [
                    asyncio.create_task(
                        _eval_one_sample(i, r, sem, judge_session, pbar)
                    )
                    for (i, r) in items
                ]
