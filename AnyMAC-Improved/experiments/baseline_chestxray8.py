...
from datasets_my.chestxray8_dataset import ChestXray8Dataset

BASE_URL = os.getenv("BASE_URL", "http://localhost:8000")
JUDGE_BASE_URL = os.getenv("JUDGE_BASE_URL", os.getenv("BASE_URL", "http://localhost:8001"))
API_KEY = os.getenv("API_KEY", "EMPTY")
RESULT_DIR = Path("result/chestxray8_baseline")

SYSTEM_PROMPT = (
    "You are a medical imaging expert. Read the question and choose the best option "
    "from A, B, C, or D. Respond with the letter and a brief explanation."
)

JUDGE_SYSTEM_PROMPT = (
    "You are an answer extractor. Read the model response and extract only the final "
    "answer option letter. Reply with ONLY one letter."
)


def build_question_text(record: dict) -> str:
    return (
...
async def judge_one(
    ...
    async with semaphore:
        prompt = (
            f"Question:\n{item['Question']}\n\n"
            f"Model response:\n{item['Response']}\n\n"
            "What is the model's final answer? Reply with ONLY one letter."
        )
        messages = [
        ...
        try:
            verdict = await call_llm(session, judge_model, messages, max_tokens=4, base_url=JUDGE_BASE_URL)
            extracted = verdict.strip().upper()
            judge_correct = len(extracted) > 0 and extracted[0] == item["Answer"].strip().upper()
        except Exception as e:
            verdict = f"[ERROR: {e}]"
            judge_correct = False
...
async def main_async(args):
    ...
    print(f"Dataset: ChestX-ray8 test — {len(records)} questions")
    ...
    regex_hits = sum(dataset.record_to_target_check(it["Answer"], it["Response"], it["Question"]) for it in items)
    n = len(items)
...
def main():
    p = argparse.ArgumentParser(description="ChestX-ray8 baseline evaluation (no routing)")
