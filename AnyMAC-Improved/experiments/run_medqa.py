"""
AnyMAC-Improved: Entry point for MedQA router training and evaluation.

Improvements over the static-routing baseline:
  1. Dynamic specialist prompts (--dynamic_prompts): judge generates question-specific guidance
  2. Partial credit rewards (--partial_credit): reasoning quality 1-5 score blended with binary
  3. Structured hint passing (--structured_hints): parsed specialist outputs with answer/confidence/reasoning

All three can be toggled independently via CLI flags.

Example:
  python experiments/run_medqa.py \
    --llm_name Qwen/Qwen3-8B \
    --judge_model Qwen/Qwen3-32B \
    --dynamic_prompts --partial_credit --structured_hints \
    --epochs 2 --train_num 300 --max_routing 3
"""
...
try:
    from GDesigner.domain.medqa_domain import MedQADomain
except Exception:
    MedQADomain = None  # type: ignore
...
def _apply_improvements(args):
    """Wire up the three improvement toggles globally."""
    ...
    if getattr(args, 'partial_credit', False):
        import experiments.train_medqa as tm
        tm.PARTIAL_CREDIT_ENABLED = True
        tm.REWARD_ALPHA = getattr(args, 'reward_alpha', 0.4)
        print(f"[improved] Partial credit rewards ENABLED (alpha={tm.REWARD_ALPHA})")
...
def build_graph(args):
    ...
    graph.routing_policy = getattr(args, "routing_policy", "learned")
    graph.fixed_route = [r.strip() for r in getattr(args, "fixed_route", "").split(",") if r.strip()]
    return graph


def main():
    p = argparse.ArgumentParser(description="AnyMAC-Improved MedQA Router")

    p.add_argument("--result_dir", type=str, default="result/medqa_improved")
    ...
    p.add_argument('--domain', type=str, default="medqa")
    ...
    p.add_argument("--dynamic_pool", action="store_true", default=False,
                   help="Enable dynamic specialist pool: judge generates 3-5 question-specific specialists per question.")
    ...
    p.add_argument("--eval_limit", type=int, default=1500,
                   help="Max number of eval questions. Default 1500.")
    ...
    p.add_argument("--routing_policy", type=str, default="learned", choices=["learned", "random", "fixed"],
                   help="Routing policy at inference time: learned (default), random, or fixed.")
    p.add_argument("--fixed_route", type=str, default="",
                   help="Comma-separated specialist route used when --routing_policy fixed, e.g. 'Cardiologist,Neurologist'.")
    ...
    limit_questions = getattr(args, 'eval_limit', 1500)

    dataset_train = MedQADataset(getattr(args, 'train_split', 'dev'))
    dataset_val = MedQADataset('test')

    print(f"Training Dataset Length: {len(dataset_train)}, Validation Dataset Length: {len(dataset_val)}")

    graph = build_graph(args)
    if args.routing_policy == "fixed" and not graph.fixed_route:
        raise ValueError("--fixed_route must be provided when --routing_policy fixed.")
    if graph.fixed_route:
        known_roles = set(SPECIALISTS)
        unknown = [r for r in graph.fixed_route if r not in known_roles]
        if unknown:
            raise ValueError(f"Unknown roles in --fixed_route: {unknown}")
    graph.entropy_beta = getattr(args, 'entropy_beta', 0.05)
    graph.eval_temperature = getattr(args, 'eval_temperature', 0.5)
    print(
        f"Graph Constructed (entropy_beta={graph.entropy_beta}, eval_temperature={graph.eval_temperature}, "
        f"routing_policy={graph.routing_policy})"
    )
    ...
    if args.zero_shot:
        ...
        graph.routing_policy = args.routing_policy
        graph.fixed_route = [r.strip() for r in args.fixed_route.split(",") if r.strip()]
        if args.routing_policy == "fixed" and not graph.fixed_route:
            raise ValueError("--fixed_route must be provided when --routing_policy fixed.")
        if graph.fixed_route:
            known_roles = set(SPECIALISTS)
