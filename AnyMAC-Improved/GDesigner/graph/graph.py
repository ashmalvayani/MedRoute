# MLP instead of similarity matching — AnyMAC-Improved
# Changes: structured hint passing, detailed routing logs
...
from GDesigner.hints.structured_hints import (
    parse_specialist_output,
    format_structured_hints,
    StructuredHint,
    clear_hint_log,
    get_hint_log,
)
...
# Global toggle for structured hints (set via run_medqa.py --structured_hints)
STRUCTURED_HINTS_ENABLED = False

# Global toggle for dynamic specialist pool (set via run_medqa.py --dynamic_pool)
DYNAMIC_POOL_ENABLED = False
DYNAMIC_POOL_JUDGE_MODEL = None

# Global toggle to disable hint chains (specialists reason independently)
NO_HINTS = False
...
class Graph(ABC):
    ...
    def __init__(self,
        ...
        self.min_routing_depth:int = min_routing_depth
        ...
        if use_transformer:
            ...
            # Task input dim depends on fusion mode
            if fusion_mode == "caption_image" and use_image_embeddings:
                # text_question(384) + caption(384) + image(image_embedding_dim)
                task_input_dim = 384 + 384 + image_embedding_dim
            elif fusion_mode == "cross_attention" and use_image_embeddings:
                # Cross-attention projects text and image separately, output is transformer_hidden_dim
                task_input_dim = 384  # text only for main projection
            elif use_image_embeddings:
                # Default concat: text(384) + image(image_embedding_dim)
                task_input_dim = 384 + image_embedding_dim
            else:
                task_input_dim = 384
            ...
            self.proj_to_transformer_dim_task = torch.nn.Linear(task_input_dim, transformer_hidden_dim)
            ...
            self.proj_to_transformer_dim_nap = torch.nn.Linear(task_input_dim, transformer_hidden_dim)
            self.proj_to_transformer_dim_nhp = torch.nn.Linear(task_input_dim, transformer_hidden_dim)
            # Cross-attention components (only created when needed)
            if fusion_mode == "cross_attention" and use_image_embeddings:
                self.img_proj = torch.nn.Linear(image_embedding_dim, transformer_hidden_dim)
                self.text_proj = torch.nn.Linear(384, transformer_hidden_dim)
                self.cross_attn = torch.nn.MultiheadAttention(
                    embed_dim=transformer_hidden_dim, num_heads=8, batch_first=True
                )
            # Routing transformer 
            ...
            # MLP for agent selection
            # The MLP takes the difference between a role embedding and the next-agent predictor (NAP) token
            # and outputs a single logit for each agent.  It uses a hidden layer of size transformer_hidden_dim//2.
            self.agent_mlp = MLP(transformer_hidden_dim, transformer_hidden_dim // 2, 1)
        ...
        self.cos_scaling = 1
        self.eval_deterministic = False  # Set True at eval to use softmax sampling (no Gumbel, no cos_scaling inflation)
        self.eval_temperature = 0.5  # Temperature for eval softmax sampling (lower = more deterministic)
        self.entropy_beta = 0.0  # Entropy regularization coefficient for routing
        self.routing_policy = "learned"  # learned | random | fixed (eval-time ablations)
        self.fixed_route = []  # ordered specialist names for routing_policy=fixed
    ...
    async def arun_next_agent_prediction(self, input: Dict[str, str], max_routing=10, temperature=1.0,
        ...
        query = input['task']
        image_path = input.get('image_path')  # None for text-only datasets

        available_roles = self.available_roles
        decision_role = self.decision_method

        # Get task embedding (384 dimensions, optionally + image embedding)
        text_embedding = torch.tensor(get_sentence_embedding(query)).detach().to(self.device).float()
        if self.use_image_embeddings and input.get('image_embedding') is not None:
            img_emb = input['image_embedding']
            if not isinstance(img_emb, torch.Tensor):
                img_emb = torch.tensor(img_emb)
            img_emb = img_emb.detach().to(self.device).float()
            # L2-normalize both embeddings to prevent magnitude mismatch
            text_embedding = F.normalize(text_embedding, dim=0)
            img_emb = F.normalize(img_emb, dim=0)
            if self.fusion_mode == "caption_image":
                # Also embed the caption text separately
                caption_query = input.get('task_caption', query)
                caption_emb = torch.tensor(get_sentence_embedding(caption_query)).detach().to(self.device).float()
                caption_emb = F.normalize(caption_emb, dim=0)
                raw_task_embedding = torch.cat([text_embedding, caption_emb, img_emb])
            elif self.fusion_mode == "cross_attention":
                # Project text and image separately, then cross-attend
                text_proj = self.text_proj(text_embedding.unsqueeze(0).unsqueeze(0))  # (1,1,H)
                img_proj = self.img_proj(img_emb.unsqueeze(0).unsqueeze(0))  # (1,1,H)
                # Text attends to image
                attn_out, _ = self.cross_attn(text_proj, img_proj, img_proj)  # (1,1,H)
                task_embedding = attn_out.squeeze(0).squeeze(0)  # (H,)
                raw_task_embedding = text_embedding  # for NAP/NHP projections
            else:
                raw_task_embedding = torch.cat([text_embedding, img_emb])
        else:
            if self.use_image_embeddings:
                raise FileNotFoundError(
                    f"Image embedding missing for image_path={input.get('image_path')!r}. "
                    "Check that the CSV image path resolves to an existing local file."
                )
            raw_task_embedding = text_embedding
        if not (self.use_image_embeddings and self.fusion_mode == "cross_attention" and input.get('image_embedding') is not None):
        ...
        # Dynamic specialist pool: generate question-specific specialists
        dynamic_pool_descriptions = {}  # role -> description (for dynamic prompts)
        if DYNAMIC_POOL_ENABLED and DYNAMIC_POOL_JUDGE_MODEL:
            # When VLM sees the image, use task_plain (no caption needed)
            pool_query = input.get('task_plain', query) if image_path else query
            panel = await generate_specialist_panel(
                judge_model=DYNAMIC_POOL_JUDGE_MODEL,
                question=pool_query,
                image_path=image_path,
            )
            if panel and len(panel) >= 2:
                print(f"[dynamic_pool] Got {len(panel)} specialists for question")
                # Build dynamic role embeddings, mappings, and available_roles
                available_roles = []
                role_embeddings = {}
                role_to_idx = {}
                idx_to_role = {}
                for i, entry in enumerate(panel):
                    ...
                    role_desc = entry['description']
                    available_roles.append(role_name)
                    role_to_idx[role_name] = i
                    idx_to_role[i] = role_name
                    dynamic_pool_descriptions[role_name] = role_desc
                    role_embeddings[role_name] = torch.tensor(
                        get_sentence_embedding(role_desc)
                    ).detach()

                # Add decision node
                decision_idx = len(available_roles)
                role_to_idx[decision_role] = decision_idx
                idx_to_role[decision_idx] = decision_role
                decision_desc = self.prompt_set.get_decision_role()
                role_embeddings[decision_role] = torch.tensor(
                    get_sentence_embedding(decision_desc)
                ).detach()
        ...
        decision_embedding = role_embeddings[decision_role].to(self.device)
        ...
        routing_count = 0
        final_answers = []
        selected_agents = set()  # prevent re-selecting the same specialist
        ...
        while routing_count <= max_routing:
            ...
            if routing_count == max_routing:
                ...
                # Compute agent logits using an MLP on the difference between role embeddings and the NAP token.
                agent_inputs = encoded_tokens[0:num_roles] - encoded_nap.unsqueeze(0)
                agent_similarity_scores = self.agent_mlp(agent_inputs).squeeze(-1)
                ...
                # Mask out already-selected specialists (set to -inf so they get ~0 probability)
                if selected_agents:
                    for idx in selected_agents:
                        agent_similarity_scores[idx] = float('-inf')

                # Force at least min_routing_depth specialist consultations
                # by masking out DecisionMaker until we reach the minimum depth
                if routing_count < self.min_routing_depth:
                    dm_idx = role_to_idx[decision_role]
                    agent_similarity_scores[dm_idx] = float('-inf')

                routing_policy = getattr(self, "routing_policy", "learned")
                # Random ablation: uniform sampling over valid actions in current dynamic space.
                if self.eval_deterministic and routing_policy == "random":
                    valid_indices = []
                    for idx, role_name in idx_to_role.items():
                        if role_name == decision_role:
                            continue
                    ...
                    if not valid_indices:
                        ...
                        chosen_agent_idx = random.choice(valid_indices)
                # Fixed-route ablation: deterministic specialist sequence, then decision maker.
                elif self.eval_deterministic and routing_policy == "fixed":
                    fixed_route = getattr(self, "fixed_route", []) or []
                    if routing_count < len(fixed_route):
                        target_role = fixed_route[routing_count]
                        if target_role in role_to_idx:
                            chosen_agent_idx = role_to_idx[target_role]
                        else:
                            chosen_agent_idx = role_to_idx[decision_role]
                    else:
                        chosen_agent_idx = role_to_idx[decision_role]
                # Eval learned routing: low-temperature softmax sampling.
                elif self.eval_deterministic:
                    eval_tau = getattr(self, 'eval_temperature', 0.5)
                    agent_probabilities = F.softmax(agent_similarity_scores / eval_tau, dim=0)
                    chosen_agent_idx = torch.multinomial(agent_probabilities, 1).item()
                else:
                ...
                selected_agents.add(chosen_agent_idx)
            ...
            if len(history_states) > 0:
                ...
                hint_probabilities = torch.sigmoid(hint_similarity_scores * 2)

                # Eval: threshold at 0.5. Train: stochastic sampling.
            ...
            hints = ""
            if NO_HINTS:
                pass  # Skip hint construction — specialists reason independently
            elif len(hint_mask) > 0 and sum(hint_mask) > 0:
                if STRUCTURED_HINTS_ENABLED:
                    # Structured hint passing: parse each specialist's output
                    selected_structured = [
                        history_states[i].get("structured_hint")
                        for i in range(len(hint_mask))
                        if hint_mask[i] == 1 and "structured_hint" in history_states[i]
                    ]
                    if selected_structured:
                        hints = format_structured_hints(selected_structured)
                    else:
                        # Fallback to raw hints if no structured data
                        selected = [(history_states[i]["role"], history_states[i]["output"]) for i in range(len(hint_mask)) if hint_mask[i] == 1]
                        for role, output in selected:
                            hints += f"\n{role}: {output} \n"
                else:
                    # Original raw hint passing — use specialist role names
                    selected = [(history_states[i]["role"], history_states[i]["output"]) for i in range(len(hint_mask)) if hint_mask[i] == 1]
                    if selected:
                        hints += "\nCollection of Other Agents' Output: \n"
                        for role, output in selected:
                            hints += f"\n{role}: {output} \n"
            ...
            if chosen_role != self.decision_method:
                ...
                # adding plain logic so the caption is only seen by the router and not the specialist call
                answer_task = input.get("task_plain") or input["task"]
                agent_input["task"] = answer_task
                agent_input.pop("task_plain", None)
                ...
                agent_kwargs = {"domain": self.domain, "llm_name": self.llm_name, "role": chosen_role}
                # Pass dynamic description if using dynamic pool
                if dynamic_pool_descriptions and chosen_role in dynamic_pool_descriptions:
                    agent_kwargs["dynamic_description"] = dynamic_pool_descriptions[chosen_role]
                agent_node = AgentRegistry.get(agent_group_type, **agent_kwargs)
                ...
                state_embedding = self.proj_to_transformer_dim_history(torch.cat([
                    role_embeddings[chosen_role].to(self.device),
                    output_embedding
                ], dim=0))
                
                # Parse structured hint from specialist output
                structured_hint = None
                if STRUCTURED_HINTS_ENABLED:
                    structured_hint = parse_specialist_output(chosen_role, agent_output)
                ...
                if structured_hint is not None:
                    history_entry["structured_hint"] = structured_hint
                history_states.append(history_entry)
                ...
                # Get the system prompt that was used (includes dynamic guidance if enabled)
                specialist_system_prompt = ""
                if hasattr(agent_node, 'constraint'):
                    specialist_system_prompt = agent_node.constraint
                # Check if _process_inputs stored a dynamic prompt
                if hasattr(agent_node, '_last_system_prompt'):
                    specialist_system_prompt = agent_node._last_system_prompt

                conv_entry = {
                    "step": routing_count,
                    "specialist": chosen_role,
                    "system_prompt": specialist_system_prompt,
                    "hints_provided": hints,
                    "response": agent_output,
                }
                if structured_hint is not None:
                    conv_entry["structured"] = {
                        "answer_choice": structured_hint.answer_choice,
                        "confidence": structured_hint.confidence,
                        "reasoning_summary": structured_hint.reasoning_summary,
                    }
                routing_results["conversations"].append(conv_entry)
            else:
                # If decision maker is chosen, use it as final answer and break
                decision_input = input.copy()

                answer_task = input.get("task_plain") or input["task"]
                decision_input["task"] = answer_task
                decision_input.pop("task_plain", None)

                if hints:
                    decision_input["hints"] = hints
                
                dm_llm = getattr(self, 'dm_llm_name', None) or self.llm_name
                decision_node = AgentRegistry.get(decision_role, **{"domain": self.domain, "llm_name": dm_llm})
                # Route DM to judge server if using a separate DM model
                if getattr(self, 'dm_llm_name', None) and hasattr(decision_node, 'llm') and hasattr(decision_node.llm, 'base_url'):
                    from GDesigner.llm.sllm_chat import JUDGE_BASE_URL
                    decision_node.llm.base_url = JUDGE_BASE_URL
                ...
                routing_results["agent_selections"].append(chosen_agent_idx)
                routing_results["agent_logits"].append(agent_similarity_scores.tolist())
                routing_results["conversations"].append({
                    "step": routing_count,
                    "specialist": "DecisionMaker",
                    "hints_provided": hints,
                    "response": final_answers[-1] if final_answers else "",
                })
        ...
        # Attach hint extraction log if structured hints were used
        if STRUCTURED_HINTS_ENABLED:
            routing_results["hint_extraction_log"] = get_hint_log()
            clear_hint_log()

        # Store dynamic pool info for gradient replay
        if dynamic_pool_descriptions:
            routing_results["dynamic_pool"] = [
                {"role": role, "description": dynamic_pool_descriptions[role]}
                for role in available_roles
            ]
    ...
    async def arun_next_agent_prediction_grad(self, gradient_input, sparse_context=0.0):
        ...
        # NOTE: to_device should be called once before the training loop, not per gradient input
        ...
        # Prepare embeddings for the task (text + optional image)
        text_embedding = torch.tensor(get_sentence_embedding(query)).detach().to(self.device).float()
        if self.use_image_embeddings and gradient_input.get('image_embedding') is not None:
            img_emb = gradient_input['image_embedding']
            if not isinstance(img_emb, torch.Tensor):
                img_emb = torch.tensor(img_emb)
            img_emb = img_emb.detach().to(self.device).float()
            ...
            img_emb = F.normalize(img_emb, dim=0)
            if self.fusion_mode == "caption_image":
                caption_query = gradient_input.get('task_caption', query)
                caption_emb = torch.tensor(get_sentence_embedding(caption_query)).detach().to(self.device).float()
                caption_emb = F.normalize(caption_emb, dim=0)
                task_embedding_raw = torch.cat([text_embedding, caption_emb, img_emb])
            elif self.fusion_mode == "cross_attention":
                task_embedding_raw = text_embedding  # for NAP/NHP
            else:
                task_embedding_raw = torch.cat([text_embedding, img_emb])
        else:
            if self.use_image_embeddings:
                raise FileNotFoundError(
                    f"Image embedding missing for image_path={gradient_input.get('image_path')!r}. "
                    "Check that the CSV image path resolves to an existing local file."
                )
            task_embedding_raw = text_embedding

        # Dynamic pool: rebuild role embeddings from stored panel
        dynamic_pool = gradient_input.get('dynamic_pool', None)
        if dynamic_pool:
            grad_role_embeddings = {}
            grad_role_to_idx = {}
            for i, entry in enumerate(dynamic_pool):
                role_name = entry['role']
                role_desc = entry['description']
                grad_role_to_idx[role_name] = i
                grad_role_embeddings[role_name] = torch.tensor(
                    get_sentence_embedding(role_desc)
                ).detach().to(self.device)
            # Add decision node
            decision_role = self.decision_method
            decision_idx = len(dynamic_pool)
            grad_role_to_idx[decision_role] = decision_idx
            decision_desc = self.prompt_set.get_decision_role()
            grad_role_embeddings[decision_role] = torch.tensor(
                get_sentence_embedding(decision_desc)
            ).detach().to(self.device)
        else:
            grad_role_embeddings = self.role_embeddings
            grad_role_to_idx = self.role_to_idx
        ...
        for step_idx in range(trace_length - 1):  # -1 because the last step is decision maker
            if self.fusion_mode == "cross_attention" and self.use_image_embeddings and gradient_input.get('image_embedding') is not None:
                img_emb_grad = gradient_input['image_embedding']
                if not isinstance(img_emb_grad, torch.Tensor):
                    img_emb_grad = torch.tensor(img_emb_grad)
                img_emb_grad = img_emb_grad.detach().to(self.device).float()
                img_emb_grad = F.normalize(img_emb_grad, dim=0)
                text_proj = self.text_proj(task_embedding_raw.unsqueeze(0).unsqueeze(0))
                img_proj = self.img_proj(img_emb_grad.unsqueeze(0).unsqueeze(0))
                attn_out, _ = self.cross_attn(text_proj, img_proj, img_proj)
                task_embedding = attn_out.squeeze(0).squeeze(0)
            else:
                task_embedding = self.proj_to_transformer_dim_task(task_embedding_raw)
            proj_roles_embeddings = []
            for role in grad_role_to_idx.keys():
                role_embedding = grad_role_embeddings[role]
                proj_role_embedding = self.proj_to_transformer_dim_role(role_embedding)
                proj_roles_embeddings.append(proj_role_embedding)
            ...
            # Calculate logits for agent selection using the MLP on the difference between role embeddings and the NAP token.
            agent_inputs = encoded_tokens[0:num_roles] - encoded_nap.unsqueeze(0)
            agent_similarity_scores = self.agent_mlp(agent_inputs).squeeze(-1)
            mean = agent_similarity_scores.mean()
            std = agent_similarity_scores.std()
            ...
            # Mask out already-selected specialists (consistent with forward pass)
            scaled_scores = self.cos_scaling * agent_similarity_scores
            num_agents = scaled_scores.shape[0]
            for prev_idx in agent_selections[:step_idx]:
                if prev_idx < num_agents:
                    scaled_scores[prev_idx] = float('-inf')

            # Get logprobs for the agent selection from the masked logits
            agent_logprobs = F.log_softmax(scaled_scores, dim=0)

            # Get the selected agent from trace (skip if agent no longer in pool)
            selected_agent_idx = agent_selections[step_idx]
            if selected_agent_idx >= num_agents:
                continue
            ...
            # Entropy regularization: penalize low-entropy (peaked) routing distributions
            if self.entropy_beta > 0:
                agent_probs = F.softmax(scaled_scores, dim=0)
                entropy = -(agent_probs * torch.log(agent_probs + 1e-10)).sum()
                step_loss = step_loss - self.entropy_beta * entropy
            ...
            if len(history_states) > 0:
                ...
                for i, hint_selected in enumerate(curr_hint_selections):
                    if i >= hint_similarity_scores.shape[0]:
                        continue  # skip — agent no longer in pool
                    # Get probability of selecting this hint (sigmoid)
                    hint_prob = torch.sigmoid(hint_similarity_scores[i] * 2)
            ...
            grad_idx_to_role = {v: k for k, v in grad_role_to_idx.items()}
            if selected_agent_idx not in grad_idx_to_role:
                # Role index mismatch (e.g. dynamic pool changed) — skip remaining steps
                break
            chosen_role = grad_idx_to_role[selected_agent_idx]
            ...
            if chosen_role != self.decision_method:
                ...
                history_states.append({
                    ...
                    "concat_embedding": torch.cat([
                        grad_role_embeddings[chosen_role].to(self.device),
                        output_embedding
                    ], dim=0),
    ...
    def to_device(self, device):
        ...
        # Move agent MLP to device if it exists
        if hasattr(self, "agent_mlp"):
            self.agent_mlp = self.agent_mlp.to(device)
        if hasattr(self, 'cross_attn'):
            self.img_proj = self.img_proj.to(device)
            self.text_proj = self.text_proj.to(device)
            self.cross_attn = self.cross_attn.to(device)
        self.device = device
    ...
    def add_to_optimizer(self, optimizer):
        ...
        # Add agent MLP parameters to optimizer if it exists
        if hasattr(self, "agent_mlp"):
            self.optimizer.add_param_group({"params": self.agent_mlp.parameters()})
        if hasattr(self, 'cross_attn'):
            self.optimizer.add_param_group({"params": self.img_proj.parameters()})
            self.optimizer.add_param_group({"params": self.text_proj.parameters()})
            self.optimizer.add_param_group({"params": self.cross_attn.parameters()})
    ...
    def save_model(self, path):
        ...
        model_state = {
            ...
            'use_image_embeddings': self.use_image_embeddings,
            'image_embedding_dim': self.image_embedding_dim,
            'min_routing_depth': self.min_routing_depth,
        ...
        if hasattr(self, 'cross_attn'):
            model_state['img_proj'] = self.img_proj.state_dict()
            model_state['text_proj'] = self.text_proj.state_dict()
            model_state['cross_attn'] = self.cross_attn.state_dict()
    ...
    def clone_for_inference(self):
        ...
        if hasattr(self, 'cross_attn'):
            new_graph.img_proj.load_state_dict(self.img_proj.state_dict())
            new_graph.text_proj.load_state_dict(self.text_proj.state_dict())
            new_graph.cross_attn.load_state_dict(self.cross_attn.state_dict())
    ...
    def load_model(path):
        ...
        graph = Graph(
            ...
            use_image_embeddings=model_state.get('use_image_embeddings', False),
            image_embedding_dim=model_state.get('image_embedding_dim', 0),
            min_routing_depth=model_state.get('min_routing_depth', 0),
            fusion_mode=model_state.get('fusion_mode', 'concat'),
        )
        ...
        if 'cross_attn' in model_state:
            graph.img_proj.load_state_dict(model_state['img_proj'])
            graph.text_proj.load_state_dict(model_state['text_proj'])
            graph.cross_attn.load_state_dict(model_state['cross_attn'])
