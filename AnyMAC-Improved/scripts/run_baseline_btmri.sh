...
LIMIT="${LIMIT:-}"
SEED="${SEED:-99}"
BTMRI_JSON="${BTMRI_JSON:-$REPO_DIR/datasets_my/btmri/btmri.json}"
TRAIN_JSON_PATH="${TRAIN_JSON_PATH:-$BTMRI_JSON}"
...
LOG="$REPO_DIR/logs/baseline_btmri_${TIMESTAMP}.log"
...
{
    echo "========================================"
    echo "  AnyMAC-Improved baseline eval (BTMRI)"
    echo "  Started      : $(date)"
    ...
    echo "  Seed         : $SEED"
    echo "  train_json   : $TRAIN_JSON_PATH (override TRAIN_JSON_PATH / BTMRI_JSON)"
    ...
    python experiments/baseline_btmri.py \
        --model "$MODEL" \
...
echo "Results directory: $REPO_DIR/result/btmri_baseline/"
