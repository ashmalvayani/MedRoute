#!/usr/bin/env bash
# PMC-VQA ablation: VLM-based pool/prompt generation (image-aware panel)
# Key difference from train_pmcvqa.sh:
#   - Uses Qwen3.5-9B (VLM) for EVERYTHING: agent, pool gen, prompt gen, judge
#   - Pool gen and prompt gen see the ACTUAL IMAGE, not a text caption
#   - No 32B needed — single model on all 8 GPUs
#   - Captions still used for router embedding only
#
# Prerequisites:
#   vLLM server with Qwen3.5-9B on all 8 GPUs:
#     CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python3 -m vllm.entrypoints.openai.api_server \
#       --model Qwen/Qwen3.5-9B --port 8000 --tensor-parallel-size 8 --enforce-eager \
#       --max-model-len 16384 --gpu-memory-utilization 0.90 --trust-remote-code \
#       --limit-mm-per-prompt '{"image": 1}' \
#       --allowed-local-media-path /path/to/datasets_my
#
# Usage:
#   bash scripts/train_pmcvqa_vlm_poolgen.sh
set -eo pipefail
...
# Point judge/pool/prompt gen to the SAME 9B VLM server
export JUDGE_BASE_URL="${JUDGE_BASE_URL:-http://localhost:8000}"

# ---- Config ----
MODEL="Qwen/Qwen3.5-9B"
# Use the SAME 9B VLM for judge/pool/prompt gen (it sees the actual image)
...
RESULT_DIR="result/pmcvqa_vlm_poolgen"
ENTROPY_BETA=0.05
EVAL_LIMIT=2100
EVAL_TEMPS="0.5"
SEED=99
...
echo "============================================"
echo "  PMC-VQA VLM Pool/Prompt Gen Training"
echo "  Agent Model: $MODEL (VLM)"
echo "  Pool/Prompt: $JUDGE_MODEL (VLM, sees image)"
echo "  Caption:     $CAPTION_MODEL (router embedding only)"
...
echo "  Improvements: dynamic_pool + dynamic_prompts (VLM-based)"
echo "  Eval temps: $EVAL_TEMPS | Eval limit: $EVAL_LIMIT"
...
for epoch in $(seq 1 "$EPOCHS"); do
    ...
    for tau in $EVAL_TEMPS; do
        ...
        if ls "$EDIR"/${DOMAIN}_*.json >/dev/null 2>&1; then
            ...
            mkdir -p "$EDIR"
            CUDA_VISIBLE_DEVICES="" python experiments/run_pmcvqa.py \
                --llm_name "$MODEL" \
                --judge_model "$JUDGE_MODEL" \
                --caption_model "$CAPTION_MODEL" \
                --model_path "$CKPT" \
