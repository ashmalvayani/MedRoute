#!/usr/bin/env bash
# PMC-VQA ablation: text caption-based routing (27B captions + question embedding)
# Vision pipeline:
#   - Qwen3.5-9B (VLM) on GPUs 0-3: agent model (sees actual images via file:// URLs)
#   - Qwen3-32B (text) on GPUs 4-7: pool/prompt generation (sees caption + question)
#   - Router (GNN, text-only): sees caption + question as text embeddings
#   - Captions generated by Qwen3.5-9B and cached per seed
#
# Prerequisites:
#   vLLM servers must be running:
...
#       --model Qwen/Qwen3-32B --port 8001 --tensor-parallel-size 4 --enforce-eager \
#       --max-model-len 4096 --gpu-memory-utilization 0.90 --trust-remote-code
#
# Usage:
#   bash scripts/train_pmcvqa.sh
set -eo pipefail
...
RESULT_DIR="result/pmcvqa_27b_captions"
ENTROPY_BETA=0.05
EVAL_LIMIT=2100
EVAL_TEMPS="0.5"
SEED=99
...
echo "============================================"
echo "  PMC-VQA Vision Pipeline Training"
echo "  Agent Model: $MODEL (VLM)"
echo "  Judge/Pool:  $JUDGE_MODEL (text-only)"
