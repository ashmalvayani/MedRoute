#!/usr/bin/env bash
# PMC-VQA ablation: hints disabled between specialists
# Same as vlm_poolgen but with --no_hints:
#   - Specialists reason independently (no anchoring from previous agents)
#   - Uses 27B VLM for pool/prompt gen (sees actual image)
#   - 32B for judge
#   - 27B captions for router embedding only
#
# Prerequisites:
#   GPU 0-3: Qwen3.5-9B VLM (port 8000) with --allowed-local-media-path
#   GPU 4-7: Qwen3-32B text (port 8001)
#
# Usage:
#   bash scripts/train_pmcvqa_no_hints.sh
set -eo pipefail
...
# Point judge/pool/prompt gen to the 27B VLM server (sees actual image)
...
RESULT_DIR="result/pmcvqa_no_hints"
ENTROPY_BETA=0.05
EVAL_LIMIT=2100
EVAL_TEMPS="0.5"
SEED=99
...
echo "============================================"
echo "  PMC-VQA NO HINTS Training"
echo "  Agent:       $MODEL (VLM)"
echo "  Judge/Pool:  $JUDGE_MODEL (VLM, sees image)"
...
echo "  Improvements: dynamic_pool + dynamic_prompts + NO_HINTS"
...
if [ "$EXISTING_EPOCHS" -ge "$EPOCHS" ]; then
    ...
    python experiments/run_pmcvqa.py \
        ...
        --seed "$SEED" \
        --dynamic_prompts --dynamic_pool --no_hints \
