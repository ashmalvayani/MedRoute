#!/usr/bin/env bash
# PMC-VQA ablation: simple/broad specialist panel titles instead of detailed sub-specialties
# Key difference from vlm_poolgen:
#- With hints (same as 62% baseline run)
#   - 27B VLM for pool/prompt/judge, 9B VLM for specialist inference
#
# Prerequisites:
...
export CUDA_VISIBLE_DEVICES="0"  # GNN is tiny (<100MB), fits in 15GB free alongside vLLM
...
# Point judge/pool/prompt gen to 27B VLM server
export JUDGE_BASE_URL="${JUDGE_BASE_URL:-http://localhost:8001}"

# ---- Config ----
MODEL="Qwen/Qwen3.5-9B"
# Use 27B VLM for judge/pool/prompt gen (sees actual image)
...
RESULT_DIR="result/pmcvqa_simple_panel"
ENTROPY_BETA=0.05
EVAL_LIMIT=2100
EVAL_TEMPS="0.5"
SEED=99
...
echo "============================================"
echo "  PMC-VQA Simple Panel Training"
echo "  Agent Model: $MODEL (VLM)"
echo "  Pool/Prompt: $JUDGE_MODEL (VLM, sees image, simple panel)"
