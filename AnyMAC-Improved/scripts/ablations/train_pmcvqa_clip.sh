#!/usr/bin/env bash
# PMC-VQA ablation: CLIP vision encoder (openai/clip-vit-large-patch14-336) instead of SigLIP
# Key difference from dm_cot:
#   - Uses --use_image_embeddings: SigLIP image embeddings replace text captions
#     in the GNN router input (concatenated with question text embedding)
#   - Uses --dm_cot: DM reasons before answering
#   - With hints
#   - 27B VLM for pool/prompt/judge, 9B VLM for specialist inference + DM
#
# Prerequisites:
#   vLLM servers:
#     GPU 0-3: Qwen3.5-9B on port 8000
#     GPU 4-7: Qwen3.6-27B on port 8001
#
# Usage:
#   bash scripts/train_pmcvqa_img_embed.sh
set -eo pipefail
...
echo "============================================"
echo "  CLIP Encoder Routing Training"
...
echo "  Improvements: dynamic_pool + dynamic_prompts + dm_cot + image_embeddings"
