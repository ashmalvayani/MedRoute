#!/usr/bin/env bash
# PMC-VQA ablation: 27B VLM for pool/prompt generation
# Key difference:
#   - Qwen3.6-27B (VLM) on GPUs 4-7: pool gen + prompt gen (sees actual image)
#   - Qwen3.5-9B (VLM) on GPUs 0-3: specialist agent (sees actual image)
#   - Caption used ONLY for router embedding
#   - No text-only 32B needed
#
# Prerequisites:
#   CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m vllm.entrypoints.openai.api_server \
#     --model Qwen/Qwen3.5-9B --port 8000 --tensor-parallel-size 4 --enforce-eager \
...
#     --model Qwen/Qwen3.6-27B --port 8001 --tensor-parallel-size 4 --enforce-eager \
#     --max-model-len 16384 --gpu-memory-utilization 0.90 --trust-remote-code \
#     --limit-mm-per-prompt '{"image": 1}' \
#     --allowed-local-media-path /path/to/datasets_my
set -eo pipefail
...
# 27B VLM for pool/prompt gen on port 8001
export JUDGE_BASE_URL="${JUDGE_BASE_URL:-http://localhost:8001}"

# ---- Config ----
MODEL="Qwen/Qwen3.5-9B"
# Use Qwen3.6-27B VLM for pool/prompt gen (sees actual image)
...
RESULT_DIR="result/pmcvqa_27b_poolgen"
ENTROPY_BETA=0.05
EVAL_LIMIT=2100
EVAL_TEMPS="0.5"
SEED=99
...
echo "============================================"
echo "  PMC-VQA 27B VLM Pool/Prompt Gen Training"
echo "  Agent Model: $MODEL (VLM, sees image)"
...
echo "  Improvements: dynamic_pool + dynamic_prompts (27B VLM)"
