#!/usr/bin/env bash
# MedMCQA baseline: vLLM base + judge (text-only), same models as train_medmcqa.sh (3.5-9B + 3.6-27B).
# Data: datasets_my/MedMCQA/medmcqa.json (split=dev|test|val), or CSVs under datasets_my/MedMCQA/data/<split>/.
set -euo pipefail
...
LOG="$REPO_DIR/logs/baseline_medmcqa_${TIMESTAMP}.log"
_MQA_DATA_DESC="datasets_my/MedMCQA (split=${SPLIT}: JSON slice or MedMCQA/data/${SPLIT}/*.csv)"
...

{
    ...
    python experiments/baseline_medmcqa.py \
        --model "$MODEL" \
...
echo "Results directory: $REPO_DIR/result/medmcqa_baseline/"
