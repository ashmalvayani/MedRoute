...
LOG="$REPO_DIR/logs/baseline_chestxray8_${TIMESTAMP}.log"
...
{
    echo "========================================"
    echo "  AnyMAC-Improved baseline eval (ChestX-ray8)"
    echo "  Started      : $(date)"
    ...
    echo "  Seed         : $SEED"
    echo "  train_json   : ${TRAIN_JSON_PATH:-(unset; default Medsets, then local CSV / CHESTXRAY8_JSON)}"
    ...
    python experiments/baseline_chestxray8.py \
        --model "$MODEL" \
...
echo "Results directory: $REPO_DIR/result/chestxray8_baseline/"
