...
RESULT_DIR="result/btmri_img_embed"
ENTROPY_BETA=0.05
EVAL_LIMIT=2000
EVAL_TEMPS="0.5"
...
TRAIN_MAX_MODEL_LEN="${TRAIN_MAX_MODEL_LEN:-}"
JUDGE_MAX_MODEL_LEN="${JUDGE_MAX_MODEL_LEN:-$MAX_MODEL_LEN}"
...
# BTMRI JSON paths: NeuroChat prefix -> ${MEDSETS_EXTRACT_DIR}/btmri_data/ unless BTMRI_PATH_PREFIX_NEW is set
...
echo "============================================"
echo "  BTMRI Image Embedding Routing Training"
echo "  Medsets root: ${MEDSETS_ROOT:-auto hf cache} (override with MEDSETS_ROOT)"
echo "  JSON override: ${TRAIN_JSON_PATH:-none}"
...
echo "  vLLM max-model-len train/judge: ${TRAIN_MAX_MODEL_LEN:-default}/$JUDGE_MAX_MODEL_LEN | gpu-memory-utilization: $VLLM_GPU_MEMORY_UTILIZATION"
...
if [ "$START_TRAIN_VLLM" = "1" ]; then
    start_vllm "$TRAIN_GPU" "$TRAIN_PORT" "$MODEL" "train" "$TRAIN_MAX_MODEL_LEN"
...
if [ "$START_JUDGE_VLLM" = "1" ]; then
    start_vllm "$JUDGE_GPU" "$JUDGE_PORT" "$JUDGE_MODEL" "judge" "$JUDGE_MAX_MODEL_LEN"
...
if [ "$EXISTING_EPOCHS" -ge "$EPOCHS" ]; then
    ...
    python experiments/run_btmri.py \
        --llm_name "$MODEL" \
