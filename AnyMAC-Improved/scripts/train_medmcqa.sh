#!/usr/bin/env bash
# MedMCQA training: dynamic pool + dynamic prompts, same vLLM IDs as MedQA / PubMedQA (seed=99)
#
# Usage:
#   bash scripts/train_medmcqa.sh
set -eo pipefail
...
RESULT_DIR="result/dynamic_pool_medmcqa"
ENTROPY_BETA=0.05
EVAL_LIMIT=1500
EVAL_TEMPS="0.7"
SEED=99
...
echo "  Baseline     : bash scripts/run_baseline_medmcqa.sh"
echo "============================================"
...
for epoch in $(seq 1 "$EPOCHS"); do
    ...
    for tau in $EVAL_TEMPS; do
        ...
        if ls "$EDIR"/${DOMAIN}_*.json >/dev/null 2>&1; then
            ...
            mkdir -p "$EDIR"
            CUDA_VISIBLE_DEVICES="" python experiments/run_medmcqa.py \
                --domain "$DOMAIN" \
                --llm_name "$MODEL" \
                --judge_model "$JUDGE_MODEL" \
                --model_path "$CKPT" \
