...
LOG="$REPO_DIR/logs/baseline_deeplesion_${TIMESTAMP}.log"
...
{
    echo "========================================"
    echo "  AnyMAC-Improved baseline eval (DeepLesion)"
    echo "  Started      : $(date)"
    ...
    echo "  Seed         : $SEED"
    echo "  train_json   : ${TRAIN_JSON_PATH:-(unset; default Medsets, then local CSV / deeplesion.json)}"
    ...
    python experiments/baseline_deeplesion.py \
        --model "$MODEL" \
...
echo "Results directory: $REPO_DIR/result/deeplesion_baseline/"
