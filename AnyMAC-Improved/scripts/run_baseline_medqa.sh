#!/usr/bin/env bash
# MedQA baseline: vLLM base + judge (text-only), same models as train_medqa.sh (3.5-9B + 3.6-27B).
# Data: datasets_my/MedQA/data/{test|dev|val} — repo CSVs, not Medsets.
set -euo pipefail
...
LOG="$REPO_DIR/logs/baseline_medqa_${TIMESTAMP}.log"
_MQA_DATA_DESC="datasets_my/MedQA/data/${SPLIT}"
...
{
    echo "========================================"
    echo "  AnyMAC-Improved baseline eval (MedQA)"
    echo "  Started      : $(date)"
    ...
    python experiments/baseline_medqa.py \
        --model "$MODEL" \
...
echo "Results directory: $REPO_DIR/result/medqa_baseline/"
