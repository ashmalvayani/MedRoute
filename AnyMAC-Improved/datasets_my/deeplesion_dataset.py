...
_DATA_DIR = Path(os.path.dirname(os.path.abspath(__file__))) / "DeepLesion" / "data"
_DEFAULT_JSON = _DATA_DIR / "deeplesion.json"


class DeepLesionDataset(ABC):
    def __init__(
        self,
        split: str,
    ...
    def _load_records(self) -> List[Dict[str, Any]]:
        ...
        if train_path and test_path:
            ...
            if source.suffix.lower() == ".csv":
                return load_csv_as_records(source, source.parent, "deeplesion")
        ...
        if not train_path and not test_path:
            ...
            local = load_local_vision_csv_records(_DATA_DIR, self._split, "deeplesion", self._seed)
    ...
    def _load_data(self, split: str, sample_n: int | None, seed: int) -> pd.DataFrame:
        ...
        print(f"[DeepLesion] Loaded {len(df)} samples from {split} split")
        return df
    ...
    def _ensure_image_embeddings(self):
        ...
        require_existing_images(self._total_df, "DeepLesion")
