...
from datasets_my.medsets_csv import extract_choice, format_options, load_medsets_records, records_to_dataframe, require_existing_images


_BASE = Path(os.path.dirname(os.path.abspath(__file__)))
_DATA_DIR = _BASE / "btmri"
_DEFAULT_JSON = Path(os.environ.get("BTMRI_JSON", str(_DATA_DIR / "btmri.json")))

# Bundled btmri.json may carry absolute paths from another machine; rewrite to local media root.
# Set BTMRI_PATH_PREFIX_OLD to whatever absolute prefix appears in your btmri.json records,
# and BTMRI_PATH_PREFIX_NEW (or MEDSETS_EXTRACT_DIR) to where the images live locally.
# When BTMRI_PATH_PREFIX_OLD is unset, paths are returned unchanged.


def _remap_btmri_path_prefix(path: str) -> str:
    if not path or not isinstance(path, str):
        return path
    path_u = path.replace("\\", "/").strip()
    old = os.environ.get("BTMRI_PATH_PREFIX_OLD", "").strip()
    if not old:
        return path
    if not old.endswith("/"):
        old = old + "/"
    if not path_u.startswith(old):
        return path

    new_raw = os.environ.get("BTMRI_PATH_PREFIX_NEW", "").strip()
    if new_raw:
        new_root = Path(new_raw).expanduser().resolve()
    else:
        base = Path(
            os.environ.get(
                "MEDSETS_EXTRACT_DIR",
                str(Path.home() / ".cache/medroute/medsets_extracted"),
            )
        ).expanduser().resolve()
        new_root = base / "btmri_data"
    new_pfx = str(new_root).rstrip("/") + "/"
    return new_pfx + path_u[len(old) :]


def _remap_btmri_records(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    for item in records:
        for key in ("figure_path", "image_path"):
            val = item.get(key)
            if isinstance(val, str) and val.strip():
                item[key] = _remap_btmri_path_prefix(val.strip())
    return records


class BtmriDataset(ABC):
    def __init__(
        self,
        split: str,
    ...
    def _load_records(self) -> List[Dict[str, Any]]:
        ...
        if train_path and test_path:
            ...
            if isinstance(data, dict):
                data = [data]
            return _remap_btmri_records(data)

        if not train_path and not test_path:
            medsets_records = load_medsets_records(
                ("BTMRI", "btmri", "BrainTumorMRI", "Brain Tumor MRI", "Brain-Tumor-MRI"),
                self._split,
                self._seed,
                dataset_name="btmri",
            )
            if medsets_records is not None:
                return _remap_btmri_records(medsets_records)
        ...
        if isinstance(data, dict):
            data = [data]
        data = _remap_btmri_records(data)
    ...
    def _load_data(self, split: str, sample_n: int | None, seed: int) -> pd.DataFrame:
        ...
        print(f"[BTMRI] Loaded {len(df)} samples from {split} split")
        return df
    ...
    def _ensure_image_embeddings(self):
        ...
        require_existing_images(self._total_df, "BTMRI")
